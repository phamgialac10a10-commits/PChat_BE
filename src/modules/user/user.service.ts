...
export class UserService {
  ...
  async findById(userId: number): Promise<{ data: any, token: any }> {
    const user = await this.db.query(
      `
      select u.id, u.fullname, u.email, u.phone, u.date_of_birth, u.is_active, u.gender, r.id as role_id, r.name as role, u.created_at, u.updated_at, u.refresh_token, u.refresh_expires_at
  ...
  async findyEmail(email: string): Promise<{ data: any, token: any}> {
    const user = await this.db.query(
      `
      select u.id, u.fullname, u.email, u.phone, u.date_of_birth, u.is_active, u.gender, r.id as role_id, r.name as role, u.created_at, u.updated_at, u.refresh_token, u.refresh_expires_at
      from users u inner join roles r on u.role_id = r.id 
  ...
  async finProfile(userId: number) {
    const user = await this.db.query(
      `
      select u.id, u.fullname, u.email, u.phone, u.date_of_birth, u.is_active, u.gender, r.id as role_id, r.name as role, u.created_at, u.updated_at, u.refresh_token, u.refresh_expires_at
      from users u inner join roles r on u.role_id = r.id 
